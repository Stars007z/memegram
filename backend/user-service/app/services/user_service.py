...
class UserService:
    ...
    async def delete_user(self, user_id: str) -> datetime:
        ...
        ts = _now()  # ✅ используем модульную функцию, локальная переменная не конфликтует
    ...
    async def check_and_process_auto_delete(self) -> tuple[int, list[str]]:
        ...
        result = await self.session.execute(
            ...
            .where(
                ...
                User.last_active < func.now() - func.make_interval(
                    days=subq.c.account_auto_delete_after_days
                ),
            )
        )
