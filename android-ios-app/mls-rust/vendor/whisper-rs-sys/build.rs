...
fn main() {
    ...
    // PATCHED for cross-compilation: `cfg!(target_os = ...)` inside a build
    // script reflects the *host*, not the actual cargo target. The original
    // crate fed the MSVC-only flag `/utf-8` to clang when cross-building from
    // a Windows host to Android, breaking the build. Use the cargo-provided
    // target env var instead.
