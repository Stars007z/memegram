...
All item-storage-service RPCs are unary-unary, so only that handler type is wrapped.
...
class LoggingInterceptor(grpc.aio.ServerInterceptor):
    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        # Only wrap unary-unary handlers
        if not handler.unary_unary:
            return handler

        original = handler.unary_unary
        method = handler_call_details.method  # e.g. "/item_storage.ItemStorageService/InitiateUpload"
