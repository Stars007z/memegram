...
def settings_to_proto(s) -> user_pb2.UserSettings:
...
class UserHandler(user_pb2_grpc.UserServiceServicer):
    ...
    async def _is_in_contacts(self, owner_user_id: str, requester_user_id: str) -> bool:
        try:
            ...
            gw = ContactsGateway()
            return await gw.is_contact(owner_user_id=owner_user_id, contact_user_id=requester_user_id)
        except Exception:
            return False
    ...
    async def _apply_privacy(
        ...
        if not is_owner and settings:
            ...
            if pv == "contacts":
                in_contacts = await self._is_in_contacts(
        ...
        proto = _build_full_profile(user, settings, is_owner)

        if (
            not is_owner
            and settings
            and settings.last_active_visible_to == "contacts"
    ...
    async def DeleteUser(self, request, context):
        ...
        async with self.get_session() as session:
            service = UserService(session)
            try:
                await service.delete_user(request.user_id)
    ...
    async def GetUserSettings(self, request, context):
        ...
        async with self.get_session() as session:
            ...
            try:
                ...
                return user_pb2.UserSettingsResponse(settings=settings_to_proto(settings))
            except ValueError as e:
                context.set_code(grpc.StatusCode.NOT_FOUND)
                context.set_details(str(e))
                return user_pb2.UserSettingsResponse()
        ...
        async with self.get_session() as session:
            ...
            try:
                ...
                kwargs = {
                    ...
                    if (getattr(request, f) != 0 if f in NUMERIC_FIELDS else request.HasField(f))
                }
